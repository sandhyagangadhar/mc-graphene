...
#'Reading the data file obtained from the MC simulations'
def get_input_data():
    ...
    input_file.close()
    return d
#'sort the y-data in descending order and sort the correspdoning x-data for each x-size(say 30 here)
#Note: point list is still list of lists
def get_sorted_points(p_input_data):
    y_sorted_list_desc = sorted(p_input_data, key=lambda k: [(k[1])], reverse=True)
#    print(y_sorted_list_desc)
    point_list = []
...
input_data = get_input_data()  #input data is a list of lists
sorted_point_list = get_sorted_points(input_data) #this is a list of coordinate points
print(len(np.array(sorted_point_list)))
sorted_point_array = np.transpose(np.array(sorted_point_list)) #this stores the sorted data in an array
print(len(sorted_point_array))
#print(input_data)
