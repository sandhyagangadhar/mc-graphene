...
def get_dumble(p_CM_array, ppt):
    return (ppt.y, ppt.y - (CCdist/2.0), ppt.y + (CCdist/2.0))
...
def get_mid_of_nbr_pts(p_CM_array):
    ...
    for i in range(0, p_CM_array.shape[0]):
        for j in range(0, p_CM_array.shape[1]):
            ...
            db_pt1 = get_dumble(p_CM_array, pt1)
...
#    dny.scatter(x_values, db_dny_list)
 ...
 #   upy.scatter(x_values, db_upy_list)
    ...
    plt.show()
'''
def plot_dbpoints(p_CM_array, db_dny_list, db_upy_list):
    x_values = []
    y_values = []
    labels = []
    for i in range(0, p_CM_array.shape[0]):
    ...
    plt.scatter(x_values,db_upy_list, c='r',marker='o',s=10)
#    plt.scatter(db_dny_list,db_upy_list)
    plt.show()
'''		
